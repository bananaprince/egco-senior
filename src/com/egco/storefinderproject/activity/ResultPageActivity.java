...
public class ResultPageActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new queryProductAsyncTask().execute("just gonna stop there and watch me burn #love #love #good #good good");
	...
	class queryProductAsyncTask extends AsyncTask<String, Integer, ArrayList<ProductModel>> {
		...
		protected ArrayList<ProductModel> doInBackground(String... params) {
			...
			try {
				...
				entity.add(new BasicNameValuePair(PARAM_Product_Hash, getHashTagList(params[0]).toString()));
				entity.add(new BasicNameValuePair(PARAM_Product_Word, getWordList(params[0]).toString()));
			...
			try {
				...
				if (success == 1) {
					...
					for (int i = 0; i < jsonDataArray.length(); i++) {
						...
						tempModel.setMerchantEmail(product.getString(RESULT_MERCHANT_EMAIL));
